document.addEventListener("DOMContentLoaded", () => {
    // Versione ottimizzata per non andare a scatti
    const creaEffettoSabbia = (idElemento) => {
        ...
        // Creiamo un frammento per non appesantire il browser
        ...
        testoOriginale.split("").forEach((carattere, indice) => {
            ...
            // Aumentiamo leggermente il distacco tra le lettere per fluidità
            span.style.transitionDelay = `${indice * 30}ms`; 
            fragment.appendChild(span);
            
            // Usiamo requestAnimationFrame per un'animazione più fluida
    ...
    // Aspettiamo mezzo secondo in più per far respirare il caricamento
    setTimeout(() => { 
        const header = document.getElementById('mainHeader');
        if(header) {
            header.classList.add('visible'); 
            creaEffettoSabbia("titolo-sabbia");
            // Il sottotitolo parte solo quando il titolo ha quasi finito
